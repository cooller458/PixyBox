    ...
    return (
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4'>
            {props.galleries.map((item, index) => (
                 <div key={item.itemId} className='w-full h-auto grid grid-cols-1 bg-[#000000] border-2 border-[#131314] dark:bg-white rounded-2xl text-white dark:text-gray-800 dark:border-2 dark:border-gray-200'>
                    ...
                    <div className="border-t-2 border-[#2E357B] dark:border-gray-200 p-4">
                        {item.isBanned?"": <button className='rounded-full bg-[#fbbe16]  text-white text-base w-full px-4 py-2 shadow-md' onClick={()=>props.children.cancelSellFucnction(item)}>⚡ {item.buttonTitle}</button>
}
                    </div>
