...
//Get The ABI
          ...
          console.log("myResellItemsResult",myResellItemsResult)
            ...
            const resellItems = await Promise.all(myResellItemsResult.map(async item=>{
              ...
              console.log(nftUrl)
              console.log(item)
              const priceToWei = web3Api.web3.utils.fromWei((item.price).toString(),"ether")
              var config = {
                method: 'get',
                url: `${nftUrl}`,
                headers: { 
    ...
    const resellItemFunction = async(item,newPrice)=>{
      ...
      if(marketContract){
        ...
        const priceToWei = web3Api.Web3.utils.toWei(newPrice,"ether")
    ...
    useEffect(() => {
        ...
        web3Api.web3&&account&&openForAdmin()
    ...
    const breadcrumbs = ["Dashboard", "Admin", "ALl NFTS"]
                    ...
                    <div className='flex flex-col space-y-6'>
                        ...
                        <div className='flex flex-row items-center space-x-4 rounded-md bg-[#000000] dark:bg-white border-2 border-[#262645] dark:border-2 dark:border-gray-200 p-4'>
                            ...
                            <div className='flex-grow flex-col'>
                                <p className='text-white dark:text-gray-800 text-xl font-bold'>All NFt Number</p>
                                <p className='text-[#53CEC7] dark:text-gray-600 text-sm font-bold'>You Marketplace has {resellItems.length} NFTS</p>
                            </div>
                        </div>

                        {/* galleries */}
                        {
                            isLoading?"ISLaoding":  <ArtGallery5 galleries={resellItems} openPriceModal={openPriceModal} closePriceModal={closePriceModal}>{{cancelSellFucnction:bannItem}}</ArtGallery5> 
